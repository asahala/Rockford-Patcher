...
The Amiga version provides more information on the themes, but is uses different names for them. If the RF file is examined (offsets 5618-58B4), it lists them in the following order along with their collectibles: hunter (gold), scuba (gems), cook (apples), player (cups), cowboy (coins), conductor (notes), astronaut (suns), gambler (clubs), doctor (hearts) and miner (diamonds).
